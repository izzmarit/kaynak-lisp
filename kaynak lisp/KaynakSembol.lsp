...
;; 1. DINAMIK WPS UZUNLUGUNU HESAPLA (UNSCALED - mm)
  (if (/= *WPS-NO* "")
    (progn
      (setq wps-no-len (strlen *WPS-NO*))
      (setq min-wps-len 9.0)            ; Minimum 9mm uzunluk
      (setq char-width-factor 2.0)      ; Her karakter icin genislik (yaklasik)
      (setq padding 3.0)                ; Toplam 3mm yanal bosluk (1.5mm sol, 1.5mm sag)
      (setq dyn-wps-len-unscaled (max min-wps-len 
                                     (+ padding (* char-width-factor wps-no-len))
                                   ))
    )
    (setq dyn-wps-len-unscaled 0.0) ; WPS yoksa uzunluk 0
  )
...
(if ent (setq obje-listesi (append obje-listesi (list ent))))
  ...
  (setq obje-listesi (append obje-listesi 
    (kaynak-metinler-ekle ref-baslangic ref-bitis ref-yon scl yukari-aci asagi-aci dyn-wps-len-unscaled)))

  ;; F.P. ve NDT sembolleri
  (if (= *TAM-NUF-FLAG* "1")
    (setq obje-listesi (append obje-listesi 
      (kaynak-tam-nufuz-ekle ref-bitis ref-yon scl (= *NDT-FLAG* "1") dyn-wps-len-unscaled)))
  )
  
  (if (= *NDT-FLAG* "1")
    (setq obje-listesi (append obje-listesi 
      (kaynak-ndt-ekle ref-bitis ref-yon scl (= *TAM-NUF-FLAG* "1") dyn-wps-len-unscaled)))
  )
...
(defun kaynak-tam-nufuz-ekle (ref-bitis ref-yon scl wps-var wps-dyn-len-unscaled / offset fp-pt ent ent-listesi txt-aci) 
  (setq ent-listesi '())
  (setq txt-aci 0.0)
  
  (if (= *TAM-NUF-FLAG* "1") ; Sadece TAM-NUFUZiyet varsa islem yap
    (progn ; *** BU PROGN BLOKU DAHA ÖNCE YANLIŞ KAPATILMIŞTI. ŞİMDİ TÜM ÇİZİMLER İÇİNDE. ***
      (if (> wps-dyn-len-unscaled 0.0) ; WPS kutusu varsa
        ;; Yeni Offset: (WPS baslangic 4.5mm) + wps-dyn-len-unscaled + (WPS sonrasi bosluk 5.5mm)
        (setq offset (* scl (+ 4.5 wps-dyn-len-unscaled 5.5))) 
        ;; WPS yoksa (Orijinal hali: ilk eleman 4.5mm'de)
        (setq offset (* scl 4.5))
      )
      
      (setq fp-pt (polar ref-bitis ref-yon offset))
...
(defun kaynak-ndt-ekle (ref-bitis yon scl fp-var wps-dyn-len-unscaled / offset ndt-pt ent ent-listesi txt-aci P_WPS_START G_WPS_FP G_FP_NDT) 
  (setq ent-listesi '())
  (setq txt-aci 0.0)
  
  ;; Sabit araliklar (Unscaled - mm)
  (setq P_WPS_START 4.5)  ; İlk eleman/not baslangici (ref-bitis'ten)
  (setq G_WPS_FP 5.5)     ; WPS kutusu sonu ile F.P./NDT merkezi arasi
  (setq G_FP_NDT 10.5)    ; F.P. merkezi ile NDT merkezi arasi
  
  (setq ndt-pt nil)

  (if fp-var ; F.P. (Tam Nufuziyet) var
    (progn
      (if (> wps-dyn-len-unscaled 0.0) ; WPS var, F.P. var
        ;; WPS -> F.P. (dinamik) -> NDT
        (setq offset (* scl (+ P_WPS_START wps-dyn-len-unscaled G_WPS_FP G_FP_NDT)))
        ;; WPS yok, F.P. var
        ;; F.P. (4.5mm) -> NDT (10.5mm)
        (setq offset (* scl (+ P_WPS_START G_FP_NDT))) ; 4.5 + 10.5 = 15.0mm
      )
    )
    ;; F.P. (Tam Nufuziyet) yok
    (progn
      (if (> wps-dyn-len-unscaled 0.0) ; WPS var, F.P. yok
        ;; WPS -> NDT (F.P. yerine)
        (setq offset (* scl (+ P_WPS_START wps-dyn-len-unscaled G_WPS_FP)))
        ;; WPS yok, F.P. yok
        (setq offset (* scl P_WPS_START)) ; İlk slot (4.5mm)
      )
    )
  )

  (if offset (setq ndt-pt (polar ref-bitis yon offset)))
  
  (if ndt-pt
    (progn
      ;; Daire ve Metin çizimi
      (setq ent (entmakex
        (list
          '(0 . "CIRCLE")
          '(100 . "AcDbEntity")
      ...
      (setq ent (entmakex
        (list
          ...
          (cons 10 ndt-pt)
          (cons 11 ndt-pt)
          (cons 40 (* scl 2.5)) ; Metin boyu 2.5mm
          (cons 1 *NDT-METIN*)
          (cons 50 txt-aci)
          '(72 . 1)
          '(73 . 2)
...
(defun kaynak-metinler-ekle (ref-baslangic ref-bitis yon scl yukari-aci asagi-aci dyn-wps-len-unscaled / txt-pt txt-aci kuyruk-pt ent-listesi kutu-p1 kutu-p2 kutu-p3 kutu-p4 kutu-p4-temp ent) 
  (setq ent-listesi '())
  (setq txt-aci 0.0)
  
  ;; WPS KUTUSU
  (if (> dyn-wps-len-unscaled 0.0)
    (progn
      ;; Kutu sol orta nokta (ref-bitis noktasindan 4.5mm uzaklikta)
      (setq kuyruk-pt (polar ref-bitis yon (* scl 4.5)))
      
      ;; Köşelerin dinamik olarak hesaplanması
      (setq kutu-p2 (polar kuyruk-pt (+ yon yukari-aci) (* scl 3.75)))   ; Sol Ust Kose (7.5mm yuksekligin yarisi)
      (setq kutu-p3 (polar kutu-p2 yon (* scl dyn-wps-len-unscaled)))    ; Sag Ust Kose (Uzunluk dinamik)
      (setq kutu-p4-temp (polar kuyruk-pt (+ yon asagi-aci) (* scl 3.75))) ; Sol Alt Kose
      (setq kutu-p4 (polar kutu-p4-temp yon (* scl dyn-wps-len-unscaled))) ; Sag Alt Kose (Uzunluk dinamik)
      
      ;; Dikdortgeni ciz (Kapali polyline)
      (setq ent (entmakex
        (list
          '(0 . "LWPOLYLINE")
          '(100 . "AcDbEntity")
          ...
          '(100 . "AcDbPolyline")
          '(90 . 4)
          '(70 . 1) ; Kapali polyline
          (cons 10 kutu-p4-temp) ; Sol Alt Kose
          (cons 10 kutu-p4)      ; Sag Alt Kose
          (cons 10 kutu-p3)      ; Sag Ust Kose
          (cons 10 kutu-p2)      ; Sol Ust Kose
        )))
      (if ent (setq ent-listesi (append ent-listesi (list ent))))

      ;; WPS Metni (kutu ortasinda)
      (setq txt-pt (polar kuyruk-pt yon (* scl (/ dyn-wps-len-unscaled 2.0)))) ; YENI MERKEZLEME
      (setq ent (entmakex
        (list
          '(0 . "TEXT")
          '(100 . "AcDbEntity")
